...
public class BoardServiceImplTest {
	...
	public void getTest() { 
		...
		//given
		BDDMockito.given(this.boardDaoImpl.selectBoardAllCount())
				  .willReturn(100);
		BDDMockito.given(this.boardDaoImpl.selectAllBoard())
				  .willReturn(List.of(new BoardVO(),new BoardVO(),new BoardVO(),new BoardVO(),
						  new BoardVO(),new BoardVO(),new BoardVO(),new BoardVO(),new BoardVO(),new BoardVO()));
		
		//Test -> when
		//가짜 bean에게 역할을 주는 것. 기능을 테스트 실행했을 떄 DB에 직접 연결하는 것이 아닌 것이기 때문에 가짜 데이터를 만들어 역할을 주는 것.
